...
const decorateLineDeletions = async (fileLines: string[], lineNumbers: number[]): Promise<string[]> => {
    const suffix = "// [!code --]"
    return suffixLines (fileLines, lineNumbers, suffix)
}

// decorate deleted lines with comments at the end "// [!code --]"
const decorateLineInsertions = async (fileLines: string[], lineNumbers: number[]): Promise<string[]> => {
    const suffix = "// [!code ++]"
    return suffixLines (fileLines, lineNumbers, suffix)
}

const suffixLines = async (fileLines: string[], lineNumbers: number[], suffix: string): Promise<string[]> => {
    //for (codeFileContent)
    //const fileLines: string[] = await FileUtils.getFileLines (codeFileContent)
    const newFileLines: string[] = []
    for (let lineNumber: number = 0; lineNumber < fileLines.length; lineNumber++) {
...
const decorateWithDiffTransformer = async (codeContent: string, lang: string): Promise<string> => {
...
const decorateInsertions = async (codeFileContent: string, insertLineNumbers: Number[], 
    lang: string) => {
...
// inspired by https://shiki.style/packages/transformers
const decorateDeletions = async (codeFileContent: string, deletedLineNumbers: Number[], 
    lang: string) => {
    const code = await codeToHtml(codeFileContent, {
        lang: lang,
        theme: 'monokai',
...
app.get('/', async (c) => {
    ...
    let diffDecoratedFileLines: string[] = await decorateLineDeletions (javaFileLines, deletedLines)

    // mark lines as added according to transformer's expected format
    diffDecoratedFileLines = await decorateLineInsertions (diffDecoratedFileLines, insertedLines)

    // collect lines into one string
    const diffDecoratedFileContent = FileUtils.joinFileLines (diffDecoratedFileLines)

    const transformedHtml = await decorateWithDiffTransformer (diffDecoratedFileContent, 'java')
    const fullPage = <FullPage diffContent = {transformedHtml}/>
...
const FullPage = (props: { diffContent?: any }) => {
    return (
       ...
       <Head />
       <body>
           <div id="main" dangerouslySetInnerHTML={createHtml (props.diffContent)} />
       </body>
       </html>
    )
